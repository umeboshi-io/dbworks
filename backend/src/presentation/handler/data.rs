...
pub async fn list_tables(
    ...
    let (conn_perm, _) = match state
        .permission_repo
        .resolve_connection_permission(&current_user, &conn_id)
        .await
    {
        Ok(p) => p,
        Err(e) => {
    ...
    if !conn_perm.can_read() {
        return (
            ...
            Json(serde_json::json!({ "error": "No access to this connection" })),
        )
            .into_response();
    }
...
pub async fn get_table_schema(
    ...
    let table_perm = match state
        .permission_repo
        .resolve_table_permission(&current_user, &conn_id, &table)
        .await
    {
        Ok(p) => p,
        Err(e) => {
    ...
    if !table_perm.can_read() {
        return (
            StatusCode::FORBIDDEN,
            Json(serde_json::json!({ "error": "No access to this table" })),
        )
            .into_response();
    }
...
pub async fn create_row(
    ...
    if !table_perm.can_write() {
        return (
            StatusCode::FORBIDDEN,
            Json(serde_json::json!({ "error": "Write access required" })),
        )
            .into_response();
    }
